...
def test_create_entity_valid_type():
    game = GameController()

    entity = game.create_entity("orc", x=1, y=2)
...
def test_create_entity_invalid_type():
    ...
    with pytest.raises(ValueError):
        game.create_entity("dragon")

def test_restart_level_resets_state():
    game = GameController()

    game.entities.append("fake_entity")
    game.player = object()
...
def test_next_level_only_if_cleared():
    game = GameController()
...
def test_next_level_when_cleared():
    game = GameController()
...
def test_level_cleared_when_no_npcs():
    game = GameController()

    game.entities = []

    game._GameController__check_game_state()

    assert game.level_cleared is True

def test_level_not_cleared_when_npcs_exist():
    game = GameController()
...
def test_update_entities_removes_dead():
    ...
    game._GameController__update_entities(0.1)

    assert e not in game.entities